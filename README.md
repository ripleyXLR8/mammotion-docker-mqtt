...
| `MAP_OFFSET` | *(vide)* | correction d'alignement `dlat,dlon` (degrés) appliquée à tout (tondeuse, base, zones). Le RTK est centimétrique en **relatif** mais l'ancrage **absolu** (position GPS brute de la base) est à quelques mètres près : ceci recale l'ensemble. Se règle en visuel sur le fond satellite (`/map?cal=1` → cliquer la vraie base affiche la valeur à mettre ici). |
